...
func (s *server) handleVerification(w http.ResponseWriter, r *http.Request, cookie, flowID string) {
	...
	flow, _, err := s.KratosAPIClient.FrontendApi.CreateBrowserVerificationFlow(ctx).Execute()
	if err != nil {
		writeError(w, http.StatusUnauthorized, err)
		return
	}
...
func NewServer(kratosPublicEndpointPort int) (*server, error) {
	...
	// base url will be kratos because we are using docker-compose
...
func (s *server) handleIndex(w http.ResponseWriter, r *http.Request) {
	// create self-service registration flow for browser
